...
from transformers.common import lockutils
from transformers.common import processutils
...
synchronized = lockutils.synchronized_with_prefix('swift-')
...
def find_config(config_path):
    ...
    :raises: `swift.exception.ConfigNotFound`

    """
    possible_locations = [
        config_path,
        os.path.join(CONF.state_path, "etc", "swift", config_path),
        os.path.join(CONF.state_path, "etc", config_path),
        os.path.join(CONF.state_path, config_path),
        "/etc/swift/%s" % config_path,
    ]
...
def cinderdir():
    from transformers import swift
    return os.path.abspath(transformers.__file__).split('swift/__init__.py')[0]
...
def monkey_patch():
    ...
    Example: 'swift.api.ec2.cloud:' \
     swift.openstack.common.notifier.api.notify_decorator'

    Parameters of the decorator is as follows.
    (See swift.openstack.common.notifier.api.notify_decorator)
...
def get_root_helper():
    return 'sudo swift-rootwrap %s' % CONF.rootwrap_config


def brick_get_connector_properties(multipath=False, enforce_multipath=False):
    """wrapper for the brick calls to automatically set
    the root_helper needed for swift.
...
def transformers_version_to_int(version):
    try:
        if isinstance(version, six.string_types):
            version = transformers_version_to_tuple(version)
...
def transformers_version_to_str(version_int):
    version_numbers = []
    factor = 1000
    while version_int != 0:
...
def transformers_version_to_tuple(version_str):
