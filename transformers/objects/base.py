...
from transformers import objects
from transformers.objects import fields
from transformers.common import versionutils
...
class CinderObject(object):
    ...
    def _obj_make_obj_compatible(self, primitive, target_version, field):
        ...
        target_version = utils.transformers_version_to_tuple(target_version)
        for index, versions in enumerate(self.obj_relationships[field]):
            my_version, child_version = versions
            my_version = utils.transformers_version_to_tuple(my_version)
    ...
    def obj_make_compatible(self, primitive, target_version):
        ...
        This may mean transformersing the format of object attributes, removing
        ...
        - If a new version changed or restricted the format of a field, this
          should transformers it back to something a client knowing only of the
...
class CinderObjectDictCompat(object):
    ...
    measure until all callers are transformersed to use modern
    attribute access.
...
def obj_make_list(context, list_obj, item_cls, db_list, **extra_args):
    ...
    :param:db_list: The list of primitives to transformers to objects
