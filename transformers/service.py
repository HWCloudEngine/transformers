...
#from transformers import db
from transformers import exception
from transformers.i18n import _, _LE, _LI, _LW
from transformers.common import loopingcall
from transformers.common import service
from transformers import rpc
from transformers import version
from transformers import wsgi
...
service_opts = [
    ...
    cfg.StrOpt('osapi_transformers_listen',
               default="0.0.0.0",
               help='IP address on which OpenStack Volume API listens'),
    cfg.IntOpt('osapi_transformers_listen_port',
               default=9900,
               help='Port on which OpenStack Volume API listens'),
    cfg.IntOpt('osapi_transformers_workers',
               help='Number of workers for OpenStack Volume API service. '
                    'The default is equal to the number of CPUs available.'), 
    cfg.StrOpt('convert_manager',
               default="transformers.server.manager.ConvertManager",
               help='Number of workers for OpenStack Volume API service. '
                    'The default is equal to the number of CPUs available.'),
    cfg.StrOpt('host',
               default="0.0.0.0",
...
def setup_profiler(binary, host):
    if CONF.profiler.profiler_enabled:
        _notifier = osprofiler.notifier.create(
            ...
            rpc.TRANSPORT, "transformers", binary, host)
...
class Service(service.Service):
    ...
    def create(cls, host=None, binary=None, topic=None, manager=None,
        ...
        #manager init
        if not manager:
            subtopic = topic.rpartition('transformers-')[2]
